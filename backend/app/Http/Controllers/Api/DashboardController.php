...
class DashboardController extends Controller
{
    public function metrics(): JsonResponse
    {
        ...
        $avgFirstResponse = DB::table('tickets')
            ...
            ->select('tickets.id', DB::raw('MIN(comments.created_at) as first_response'), 'tickets.created_at as ticket_created')
            ->groupBy('tickets.id', 'tickets.created_at')
            ->get()
            ->avg(function ($row) {
                return \Carbon\Carbon::parse($row->ticket_created)
                    ->diffInMinutes(\Carbon\Carbon::parse($row->first_response));
            });
